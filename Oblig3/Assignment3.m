...
fileID = fopen('/home/preben/repo/inf379/Oblig3/Call_035_Vehicle_07.txt');
fgetl(fileID);
...
for j = 1:s
    ...
    for i = 1:n
        ...
        new_sol = operator(randi(4), ini_sol, C, V);
        T0 = 1000000;
        Tf = 1;
        T=T0-i*((T0-Tf)/n);
    ...
    %storing result..
    results(1,j+1) = ini_obj;
    
    
end

%storing sum for average..
results(1,2+s) = results(1,2+s)+ini_obj;

%calculating average..
results(1,2+s) = sum(results(1,2:s+1))/10;
...
clearvars -except vehicle_spec call_spec route_spec load_spec V C veh_call N ini_sol ini_obj results a b c d
